...
public class TaxRateParser 
{
			...
			ReceiptConfiguration receiptConfig)
	{
		...
		try
		{
	        ...
	        Map<Integer, TaxRateConfigurationItem> configurationItemsMap = new 
	        		TreeMap<Integer, TaxRateConfigurationItem>();
	        
	        for( TaxRateConfigurationItem configurationItem : receiptConfig.getTaxRate().getConfigurationItems() )
	        {
	        	configurationItemsMap.put(configurationItem.getPriority(), configurationItem);
	        }
	        
	        for( Map.Entry<Integer, TaxRateConfigurationItem> configurationItemEntry : configurationItemsMap.entrySet() )
	        {
	        	TaxRateConfigurationItem configurationItem = configurationItemEntry.getValue();
	...
	protected BigDecimal handleDataWithJsonPath(
			DocumentContext jsonContext, 
			TaxRateConfigurationItem configurationItem)
	{
		String cMethodName = "";
