...
public class TotalParser 
{
			...
			ReceiptConfiguration receiptConfig)
	{
		...
		try
		{
	        ...
	        Map<Integer, TotalConfigurationItem> configurationItemsMap = new 
	        		TreeMap<Integer, TotalConfigurationItem>();
	        
	        for( TotalConfigurationItem configurationItem : receiptConfig.getTotal().getConfigurationItems() )
	        {
	        	configurationItemsMap.put(configurationItem.getPriority(), configurationItem);
	        }
	        
	        for( Map.Entry<Integer, TotalConfigurationItem> configurationItemEntry : configurationItemsMap.entrySet() )
	        {
	        	TotalConfigurationItem configurationItem = configurationItemEntry.getValue();
	...
	protected BigDecimal handleDataWithJsonPath(
			DocumentContext jsonContext, 
			TotalConfigurationItem configurationItem)
	{
		String cMethodName = "";
